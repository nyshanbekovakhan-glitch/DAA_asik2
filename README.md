...
The Linked List time grows accordingly, roughly in proportion to n: 7.1 ms for n = 1000, 75.5 ms for n = 10000 and 625.3 ms for n = 100000.

The Dynamic Array time does not grow with n in a meaningful way, and it is not even ordered (129 µs for n = 100, 4 µs for n = 10000, 31 µs for n = 100000). The total time of 10000 `get` calls is very small, so a single run is easily disturbed by JIT compilation and garbage collection, especially for the smallest inputs. The access counter shows the real behaviour: exactly 10000, one per `get`.

### Graph

The graph was created in Excel from the benchmark results. The horizontal axis (n) is logarithmic. Series 1 (blue) is the Dynamic Array and Series 2 (red) is the Linked List. The Dynamic Array line is almost flat at the bottom because its times are very small compared with the Linked List.
...
Both structures do the same number of comparisons, but for n ≥ 1000 the Dynamic Array is about 4 to 8 times faster (26.4 ms and 119.3 ms for n = 100000). The elements of the array are stored next to each other in memory, while the Linked List must follow references to nodes that can be far apart. This is a constant-factor difference; the asymptotic complexity is the same. For n = 100 the times are almost equal (235 µs and 232 µs), because the total time is too small to be measured reliably.

### Graph

The graph was created in Excel from the benchmark results. Series 1 (blue) is the Dynamic Array and Series 2 (red) is the Linked List.
...
For n = 100000 the operations at the beginning are more than 1000 times faster for the Linked List (3.2 µs against 6.1 ms for insertion and 1.6 µs against 5.9 ms for removal).

In the middle, the Linked List made about half as many counted accesses as the Dynamic Array (50001000 against 101000000), but it was still about 21 to 22 times slower (67.4 ms and 67.2 ms against 3.1 ms and 3.0 ms). Shifting elements in an array is a sequential memory copy, while traversing a list is pointer chasing through memory. The access counts show the asymptotic behaviour, but they do not show the real cost of one access.

Some times for small n look wrong (for example, the Dynamic Array insertion at the beginning for n = 100 takes 878 µs, and 123 µs for n = 1000). This is the same JIT and measurement noise as in Workload 1 (see section 11.3).

### Graph

The graph was created in Excel from the benchmark results. It shows the insertion times only (the removal times are in the table above). Series 1 is Dynamic Array insertion at the start, Series 2 is Linked List insertion at the start, Series 3 is Dynamic Array insertion in the middle and Series 4 is Linked List insertion in the middle. The lines are smoothed by Excel, so the table gives the exact values.
...
Therefore, extraction is more expensive than insertion: for n = 100000 it needs 12.4 times more comparisons and 6.6 times more time (6.5 ms and 1.0 ms).
...
The graph was created in Excel from the benchmark results. Series 1 (blue) is the total time of n insertions and Series 2 (red) is the total time of n extractions.
...
The strongest increase can be seen in Linked List random access because every access may require traversing many nodes (0.6 seconds for 10000 accesses at n = 100000).
...
This can be seen in the results. For example, 10000 Dynamic Array `get` calls take 129 µs for n = 100, 74 µs for n = 1000 and 4 µs for n = 10000, although every case performs exactly 10000 accesses. The benchmark has no warm-up phase, so the first (smallest) cases are measured while the JIT compiler is still working. The same effect is visible in Workload 3 for n = 100.
...
Example from the experiments: the search in Dynamic Array and in Linked List performs exactly the same number of comparisons, but for n = 100000 the Linked List needs 119.3 ms and the Dynamic Array only 26.4 ms.
...
These different memory organizations affect memory access and cache behavior. In Workload 3 (middle position, n = 100000) the Linked List makes half as many counted accesses as the Dynamic Array, but is 21 to 22 times slower.
...
The graphs were created in Excel from the measured benchmark results.

The tables in sections 7-10 and the graphs come from the same benchmark run. The CSV files contain the result of another run of the same benchmark: the operation counters (accesses and comparisons) are identical, but the execution times are different, because the measured time changes from run to run.
